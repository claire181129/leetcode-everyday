...
* 3, 
*/
unsigned int do_change(unsigned int value, unsigned int change_unit)
{
    return value + change_unit;
}
void generate_list(unsigned int n, unsigned int* plist)
{
    unsigned int number_count = n * (n + 1) / 2;
    list = (unsigned int*)malloc(sizeof(unsigned int) * number_count);
    unsigned int start_value = 1;
...
void print_matrix(unsigned int** pmatrix)
{
